...
class Simple_Local_Avatars {
	...
	public function admin_init() {
		...
		add_settings_field( 'simple-local-avatars-caps', __('本地头像权限','simple-local-avatars'), array( $this, 'avatar_settings_field' ), 'discussion', 'avatars' );
	}
	...
	public function avatar_settings_field( $args ) { 
		...
		echo '
			<label for="simple_local_avatars_caps">
				...
				' . __('仅作者及更高等级用户才具有设置本地头像权限。','simple-local-avatars') . '
			</label>
		';
	}
			...
			<?php
					if ( empty( $profileuser->simple_local_avatar ) )
						...
						echo '
							...
							<span class="description">' . __('重新上传和移除本地头像，都需选中“移除本地头像”复选框，并更新个人资料即可。','simple-local-avatars') . '</span>
						';		
				} else {
					if ( empty( $profileuser->simple_local_avatar ) )
						echo '<span class="description">' . __('尚未设置本地头像，请在 Gravatar.com 网站设置头像。','simple-local-avatars') . '</span>';
					else 
						echo '<span class="description">' . __('你没有头像上传头像，如需要修改本地头像，请联系站点管理员。','simple-local-avatars') . '</span>';
				}
			?>
			</td>
		</tr>
